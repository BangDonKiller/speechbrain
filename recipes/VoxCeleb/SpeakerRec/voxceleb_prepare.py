...
def prepare_voxceleb(
    ...
    max_train_csv_rows=None,
    max_dev_csv_rows=None,
    skip_prep=False,
):
    """
    ...
    test_data_folder : str
        Path to a separate folder containing test wav files.
        If None, test data is searched in data_folder.
    split_speaker : bool
        Speaker-wise split
    random_segment : bool
        Train random segments
    max_train_csv_rows : int
        Maximum number of training rows to write into train.csv.
        If None, all rows are written.
    max_dev_csv_rows : int
        Maximum number of development rows to write into dev.csv.
        If None, all rows are written.
    skip_prep : bool
    ...
    conf = {
        ...
        "max_train_csv_rows": max_train_csv_rows,
        "max_dev_csv_rows": max_dev_csv_rows,
    }
    ...
    if test_data_folder is None:
        test_data_folders = data_folder
    elif "," in test_data_folder:
        test_data_folders = test_data_folder.replace(" ", "").split(",")
    else:
        test_data_folders = [test_data_folder]
    ...
    if "train" in splits:
        prepare_csv(
            ...
            random_segment,
            amp_th,
            max_entries=max_train_csv_rows,
        )
    ...
    if "dev" in splits:
        prepare_csv(
            ...
            random_segment,
            amp_th,
            max_entries=max_dev_csv_rows,
        )

    # For PLDA verification
    if "test" in splits:
        prepare_csv_enrol_test(
            test_data_folders, save_folder, verification_pairs_file
        )
    ...
    for data_folder in data_folders:
        ...
        if split_speaker:
            ...
            for f in audio_paths:
                spk_id = _extract_spk_id_from_wav_path(f)
                if spk_id is None:
...
def _list_voxceleb_audio_files(data_folder):
    ...
    for spk_dir in progress:
        ...
        session_pattern = os.path.join(spk_dir, "*")
        for session_dir in glob.glob(session_pattern):
...
def _extract_spk_id_from_wav_path(wav_path):
    """Extract speaker id from an absolute wav path under .../wav/idXXXX/..."""
    normalized = os.path.normpath(wav_path)
    parts = normalized.split(os.sep)
    try:
        wav_idx = parts.index("wav")
        return parts[wav_idx + 1]
    except (ValueError, IndexError):
        return None


def _extract_spk_sess_utt_from_wav_path(wav_path):
    """Extract (speaker, session, utterance) from absolute wav path."""
    normalized = os.path.normpath(wav_path)
    parts = normalized.split(os.sep)
    try:
        wav_idx = parts.index("wav")
        spk_id = parts[wav_idx + 1]
        sess_id = parts[wav_idx + 2]
        utt_id = parts[wav_idx + 3]
        return spk_id, sess_id, utt_id
    except (ValueError, IndexError):
        return None


def _audio_path_candidates_from_rel_id(data_folder, rel_id):
    """Build candidate audio paths from a VoxCeleb relative id like id10270/xxx/00001."""
    rel_parts = rel_id.split("/")
    base_path = os.path.join(data_folder, "wav", *rel_parts)
    return [base_path + extension for extension in AUDIO_EXTENSIONS]


def _resolve_wav_from_rel_id(data_folders, rel_id):
    """Resolve an audio path from list of possible dataset roots."""
    for data_folder in data_folders:
        for audio_path in _audio_path_candidates_from_rel_id(data_folder, rel_id):
            if os.path.exists(audio_path):
                return audio_path
    return _audio_path_candidates_from_rel_id(data_folders[0], rel_id)[0]
...
def prepare_csv(
    ...
    max_entries: int
        Maximum number of rows (excluding header) to write.
        If None, writes all rows.
    """
    ...
    for wav_file in tqdm(wav_lst, dynamic_ncols=True):
        ...
        path_info = _extract_spk_sess_utt_from_wav_path(wav_file)
        if path_info is None:
            logger.info(f"Malformed path: {wav_file}")
            continue
        spk_id, sess_id, utt_id = path_info
...
def prepare_csv_enrol_test(data_folders, save_folder, verification_pairs_file):
    ...
    for data_folder in data_folders:
        ...
        enrol_csv = []
        for id in enrol_ids:
            wav = _resolve_wav_from_rel_id(data_folders, id)
            ...
            start_sample = 0
            stop_sample = signal.shape[0]
            path_info = _extract_spk_sess_utt_from_wav_path(wav)
            if path_info is None:
                logger.info(f"Malformed path: {wav}")
                continue
            spk_id, sess_id, utt_id = path_info
